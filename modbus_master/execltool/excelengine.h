...
public:
    ...
    void Save();                                              //保存xls报表
    void Close();                                             //关闭xls报表
    bool SaveDataFrTable(QTableWidget* tableWidget,int mode);          //保存数据到xls
    bool SaveDataFrTableByLineData(QTableWidget *tableWidget,int lineNum);
    ...
    bool     SetCellData(const QString& cellw,QVariant data);
     void SetPic(void);
   void  mergeCells(const QString& cell);
   void  setCellTextCenter(int row, int column);
   void  setCellTextCenter(const QString& cellw);
   void  setCellTextColor(int row, int column);
