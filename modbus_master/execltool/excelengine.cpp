...
/**
  ...
  *                   false: 失败
  */
bool ExcelEngine::SaveDataFrTable(QTableWidget *tableWidget,int mode)
{
    if(NULL == tableWidget)
    {
        return false;
    ...
    if(mode == 1)
    {
        ...
        for(int i=0;i<tableR;i++)
        {
            for(int j=0;j<tableC;j++)
            {
                ...
                {
                    this->SetCellData(i+6+16,j+1,tableWidget->item(i,j)->text());
                }
            }
        }
    ...
    for(int i=0;i<tableR;i++)
    {
        for(int j=0;j<tableC;j++)
        {
            if(tableWidget->item(i,j) != NULL)
            {
                this->SetCellData(i+2+16,j+1,tableWidget->item(i,j)->text());
            }
        }
    }
    }
...
bool ExcelEngine::SaveDataFrTableByLineData(QTableWidget *tableWidget,int lineNum)
{
    if(NULL == tableWidget)
    {
        return false;
...
bool ExcelEngine::SetCellData(const QString& cellw,QVariant data)
{
    bool op = false;
    QAxObject *cell = pWorksheet->querySubObject("Range(const QString&)",cellw);//获取单元格对象
    if ( cell )
    {
        QString strData = data.toString(); //excel 居然只能插入字符串和整型，浮点型无法插入
        cell->dynamicCall("SetValue(const QVariant&)",strData); //修改单元格的数据
...
void ExcelEngine::SetPic()
{
    QAxObject * shapes = pWorksheet->querySubObject("Shapes");

    QString dirname = QCoreApplication::applicationDirPath().replace(QString("/"), QString("\\"));
    QString picDir=(dirname+"\\chart.png");
    shapes->dynamicCall("AddPicture( QString&, bool, bool, double, double, double, double",picDir,true,true,20,10,800,230);
}
...
void ExcelEngine::setCellTextCenter(const QString& cellw)
{
//    QString cell;
//    cell.append(QChar(column - 1 + 'A'));
//    cell.append(QString::number(row));

    QAxObject *range = pWorksheet->querySubObject("Range(const QString&)", cellw);
    range->setProperty("HorizontalAlignment", -4108);//xlCenter
}

void ExcelEngine::setCellTextColor(int row, int column)
{
    QString cell;
    cell.append(QChar(column - 1 + 'A'));
    cell.append(QString::number(row));
